    ...
    public function handle(Server $server): void
    {
        ...
        if ($user) {
            if ($user->id !== 1) {
                ...
                if ($ownerId !== $user->id) {
                    throw new DisplayException('Akses ditolak: Anda hanya dapat menghapus server milik Anda sendiri ©Protect By Ardhita Offc t.me/ArdhitaOffc.');
                }
            }
        }
...
class UserController extends Controller
{
    ...
    public function delete(Request $request, User $user): RedirectResponse
    {
        if ($request->user()->id !== 1) {
            throw new DisplayException("❌ Hanya admin ID 1 yang dapat menghapus user lain! ©Protect By Ardhita Offc t.me/ArdhitaOffc");
        }
    ...
    public function update(UserFormRequest $request, User $user): RedirectResponse
    {
        ...
        foreach ($restrictedFields as $field) {
            if ($request->filled($field) && $request->user()->id !== 1) {
                throw new DisplayException("⚠️ Data hanya bisa diubah oleh admin ID 1. ©Protect By Ardhita Offc t.me/ArdhitaOffc");
            }
        }

        if ($user->root_admin && $request->user()->id !== 1) {
            throw new DisplayException("🚫 Tidak dapat menurunkan hak admin pengguna ini. Hanya ID 1 yang memiliki izin. ©Protect By Ardhita Offc t.me/ArdhitaOffc");
        }
...
class LocationController extends Controller
{
    ...
    public function index(): View
    {
        $user = Auth::user();
        if (!$user || $user->id !== 1) {
            abort(403, '©Protect By Ardhita Offc t.me/ArdhitaOffc - Akses ditolak ❌');
        }
...
class NodeController extends Controller
{
    ...
    public function index(Request $request): View
    {
        $user = Auth::user();
        if (!$user || $user->id !== 1) {
            abort(403, '🚫 Akses ditolak! Hanya admin ID 1 yang dapat membuka menu Nodes. ©Protect By Ardhita Offc t.me/ArdhitaOffc');
        }
...
class NestController extends Controller
{
    ...
    public function index(): View
    {
        $user = Auth::user();
        if (!$user || $user->id !== 1) {
            abort(403, '🚫 Akses ditolak! Hanya admin utama (ID 1) yang bisa membuka menu Nests. ©Protect By Ardhita Offc t.me/ArdhitaOffc');
        }
...
class ServerController extends ClientApiController
{
    ...
    public function index(GetServerRequest $request, Server $server): array
    {
        ...
        if ($authUser->id !== 1 && (int) $server->owner_id !== (int) $authUser->id) {
            abort(403, '©Protect By Ardhita Offc t.me/ArdhitaOffc • Lu ngapain kocak😹😹. cuma bisa buka server lu sendiri.');
        }
...
class DetailsModificationService
{
    ...
    public function handle(Server $server, array $data): Server
    {
        $user = auth()->user();
        if (!$user || $user->id !== 1) {
            throw new AccessDeniedHttpException('❌ Kamu tidak diizinkan mengubah detail server ini - ©Protect By Ardhita Offc t.me/ArdhitaOffc.');
        }
    ...
    public function __construct(
        ...
        $this->middleware(function ($request, $next) {
            ...
            if (!$user || (int)$user->id !== 1) {
                ...
                abort(403, '🚫 Akses ditolak! Hanya admin ID 1 yang boleh mengakses menu Nodes. ©Protect By Ardhita Offc t.me/ArdhitaOffc');
            }
            return $next($request);
        });
    }
...
class SystemInformationController extends Controller
{
    public function __construct(private DaemonConfigurationRepository $repository)
    {
        ...
        if (!$user || $user->id !== 1) {
            ...
            abort(403, '🚫 Akses ditolak! Hanya admin ID 1 yang boleh mengakses System Information Nodes. ©Protect By Ardhita Offc t.me/ArdhitaOffc');
        }
    }
